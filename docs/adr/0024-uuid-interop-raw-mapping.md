...
> **Correction (2026-06-26):** UUID interop has shipped. `toUUID`, `fromUUID`, and `safeFromUUID` are available on every codec instance; the `"invalid_uuid"` `ParseError` reason is part of the stability contract. The CLI surface also shipped: `inspect` emits a `uuid:` line, `generate --uuid` emits raw UUID output, and `inspect --from-uuid <uuid> --brand <brand>` converts back. The "deferred to follow-up issues" and "README left untouched until the feature ships" statements in the body are now historical. The deferred items listed in Consequences (spec-valid `toUUIDv7`, version-asserting `fromUUIDv7`, wider input leniency, ORM native-uuid storage mode, SQL DDL emitter, `SPEC.md` conformance vectors) remain deferred.
