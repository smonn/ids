...
- **`released:` is keyed on the major version *line*, not the semver version.** `released:v1` (later `v2`) is multi-select so a back-ported fix shipped in two lines carries both. Per-semver-version labels (`released:1.4.0`) were rejected — see Considered options.
