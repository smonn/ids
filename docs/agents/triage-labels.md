...
> **Ownership — App only, not agents** (with one exception). Every label in the state machine below (and `ready-for-agent` / `ready-for-human` / `needs-triage` / `needs-info` / `wontfix` above) is a _pipeline_ label owned by the `.github/workflows/` automations. **Agents must not set or remove them.** When a blocker closes, `unblock.yml` flips `blocked → needs-triage` and triage re-evaluates — it never jumps an issue straight to `ready-for-agent`, so doing that by hand both usurps the App and lands the wrong state. A `PreToolUse` hook (`.claude/settings.json` → `.claude/hooks/guard-pipeline-labels.mjs`) denies any `mcp__github__issue_write` that includes one of these labels. Edit issue body/title/state as needed; leave the lifecycle labels to the App.
>
> **Exception:** `address-feedback` and `needs-review` are designated "maintainer-applied" (see the table below). Any agent session may set these two labels to drive the review feedback loop — actor identity cannot be verified in a PreToolUse hook, so the trade-off is accepted because they control review lifecycle, not triage lifecycle. The hook does not block them — they are absent from the LIFECYCLE set by omission, not via a positive allowlist.
