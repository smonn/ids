...
>
> **Exception:** `address-feedback` and `needs-review` are designated "maintainer-applied" (see the table below). A monitoring agent session acting on behalf of the maintainer may set these two labels on agent PRs to drive the review feedback loop. The hook allows them explicitly.
