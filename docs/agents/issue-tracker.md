...
2. **Slice for the pipeline, not just for humans:**
   ...
   - **Write class-level acceptance criteria for pattern findings.** When a finding is an instance of a repeating pattern (an unredacted echo site, a missed sibling file, a scattered literal), the AC must name the class and require a repo-wide sweep — "grep for every site where X; fix each or list the skipped ones in the PR" — with the known sites given as a starting point, not the full spec. Site-enumerated ACs are the top producer of next-round audit findings (see CONTRIBUTING "Scope findings by class").
