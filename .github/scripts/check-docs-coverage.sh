#!/usr/bin/env bash
# check-docs-coverage.sh — advisory source→website-docs coverage mapping for PRs.
#
...
#
# It is PURELY ADVISORY: it never exits non-zero on a gap (only on its own
# misuse). The workflow turns the report into a sticky PR comment.
#
...
if [ "$gaps" -gt 0 ]; then
  {
    ...
    echo
    echo 'This PR changes source whose hand-written docs page was not updated in the same diff. The TypeDoc API reference regenerates itself, but these narrative pages do not — please update them, or note in the PR why no change is needed.'
    echo
    echo '| Changed source | Expected docs page |'
    echo '| --- | --- |'
    printf '%s' "$rows"
    echo
    echo '_Advisory only — this check never blocks merge. See `CONTRIBUTING.md` > Style for the source↔docs mapping._'
  } >>"$REPORT_FILE"
fi
