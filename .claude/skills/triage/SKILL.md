...
1. **Gather context.** Read the full issue or PR (body, comments, labels, author, dates; for a PR, the diff too). Parse any prior triage notes so you don't re-ask resolved questions. Explore the codebase using the project's domain glossary, respecting ADRs in the area. Run three checks against the codebase: (a) **Wire-changing check** — determine whether the issue's resolution would require touching `spec/vectors.json`, `SPEC.md`, or a frozen codec construction (an ADR-locked byte layout, canonical-form rule, or codec variant construction — see [ADR-0025 § "Decision: a three-layer guard against automated un-freezing"](docs/adr/0025-frozen-wire-spec-conformance-vectors.md#decision-a-three-layer-guard-against-automated-un-freezing)). If any of these paths or constructions are in scope, classify `ready-for-human`, never `ready-for-agent`; proceed directly to step 5 without completing the remaining checks. (b) **Redundancy** — search for an existing implementation of the requested behavior by domain concept (not just the request's wording), and report where you looked. If found, it's an already-implemented `wontfix` (step 5). (c) **Prior rejection** — read `.out-of-scope/*.md` and surface any that resembles this request.
