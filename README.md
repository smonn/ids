...
- `IdsErrorCode` — String-literal union of the twelve stable error codes carried by `IdsError` (`invalid_brand`, `invalid_key_format`, `invalid_key_encoding`, `invalid_key_length`, `invalid_kind`, `empty_keyring`, `duplicate_keyring_entry`, `invalid_lookup_key`, `verification_failed`, `invalid_id`, `invalid_namespace`, `invalid_timestamp`).
