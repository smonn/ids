...
**Exception — address-feedback / needs-review:** Any agent session may set `address-feedback` and `needs-review` on agent PRs — the hook does not restrict by role. These two labels are designated "maintainer-applied" in `docs/agents/triage-labels.md`; the hook does not block them — they are absent from the LIFECYCLE set by omission, not via a positive allowlist. (The primary use-case is a monitoring session driving the review feedback loop, but the capability is not scoped to that role.)
