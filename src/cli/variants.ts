...
import type { Id, StandardSchemaProps, ValidBrand } from "../types.js";
...
type InspectCapability =
  ...
  | {
      ...
      safeVerify(codec: IdCodec<ValidBrand>, id: string): Promise<SafeVerifyResult<ValidBrand>>;
    }
...
function standardValidate(
  ...
  const result = (codec as unknown as { "~standard": StandardSchemaProps<ValidBrand> })[
    "~standard"
  ].validate(input);
