...
Run `pnpm test`, `pnpm typecheck`, `pnpm lint`, `pnpm fmt:check`, `pnpm depcruise`, and `pnpm knip` before opening a PR. A Husky pre-push hook runs the fast static tier (`label-trigger-lint`, `adr-front-matter-lint`, `source-hygiene-lint`, `knip`, `depcruise`, `lint`, `fmt:check`, `typecheck`, `build`) automatically on `git push`; use `--no-verify` to bypass in an emergency.
...
- **Mutation testing runs weekly in CI** (`mutation.yml`, Stryker over `src/` + `bin/`) and fails on score collapse: `thresholds.break` in `stryker.config.json` makes the run exit non-zero below the floor (ADR-0037; the posture is pinned by `test/mutation-gating.test.ts`). Individual surviving mutants above the floor are still triaged into issues, not red checks. When touching poorly-scored code, `pnpm mutation` with a narrowed scope (`npx stryker run --mutate "src/cli/flags.ts"`) gives a fast local signal.
